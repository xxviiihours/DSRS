...
public class CreateItemEndpoint(IMediator mediator) : Endpoint<CreateItemRequest, IResult>
{
    ...
    public override async Task<IResult> ExecuteAsync(CreateItemRequest request, CancellationToken ct)
    {
        ...
        return result.ToHttpResult(
            mapResponse => new CreateItemResponse(result.Data!.Id.ToString(), result.Data!.Name),
            locationBuilder => $"{CreateItemRequest.Route}/{result.Data?.Id}",
...
public class CreateItemResponse(string id, string name)
{
